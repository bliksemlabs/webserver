...
static void
json_location (json_t *j, char *key, rtime_t arrival, rtime_t departure,
               latlon_t location, time_t date) {
    json_key_obj(j, key);
        json_kv(j, "name", NULL);
       	json_kf(j, "lat", location.lat);
       	json_kf(j, "lon", location.lon);
...
otp_json(json_t *j, plan_t *plan, tdata_t *tdata, char *buf, uint32_t buflen) {
    ...
    json_obj(j);
        ...
        json_key_obj(j, "plan");
            ...
            if (plan->req.from_stop_point == STOP_NONE) {
	    	json_location(j, "from", UNREACHED, UNREACHED, plan->req.from_latlon, date_seconds);
	    } else {
	    	json_place(j, "from", UNREACHED, UNREACHED, plan->req.from_stop_point, tdata, date_seconds);
            }

            if (plan->req.to_stop_point == STOP_NONE) {
	    	json_location(j, "to", UNREACHED, UNREACHED, plan->req.to_latlon, date_seconds);
	    } else {
